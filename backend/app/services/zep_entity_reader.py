...
class ZepEntityReader:
    ...
    2. Filter nodes that match predefined entity types (nodes with labels beyond just Entity)
    ...
    def get_all_edges(self, graph_id: str) -> List[Dict[str, Any]]:
        """
        Get all edges from the graph (with pagination)
    ...
    def filter_defined_entities(
        ...
        Filtering logic:
        - If a node's Labels only contain "Entity", it means this entity does not match our predefined types; skip it
        - If a node's Labels include labels other than "Entity" and "Node", it matches a predefined type; keep it
        ...
        for node in all_nodes:
            ...
            if enrich_with_edges:
                ...
                # Get basic information about related nodes
                related_nodes = []
        ...
        logger.info(f"Filtering complete: total nodes {total_count}, matching entities {len(filtered_entities)}, "
    ...
    def get_entity_with_context(
        ...
        """
        try:
            # Use retry mechanism to get the node
