...
4. Support user conversation with autonomous tool invocation
"""
...
This tool is used to get a complete overview of simulation results, especially suited for understanding event evolution. It will:
...
PLAN_SYSTEM_PROMPT = """\
You are an expert in writing "Future Prediction Reports" with a "God's Eye View" of the simulated world — you can observe every Agent's behavior, speech, and interactions.
...
- Agent speech and behavior in the simulated world are predictions of future group behavior
...
3. [Language Consistency - Quoted Content Must Be Translated to Report Language]
   ...
   - If the simulation requirement and source materials are in the same language, the report must be written entirely in that language
   - When quoting content returned by tools, translate it into fluent report language before writing it in
...
4. [Faithfully Present Prediction Results]
   ...
   - If information is insufficient in some areas, state so honestly
...
Platform A served as the primary venue for public opinion, carrying the core function of information first release:
...
[Tool Usage Tips - Please mix different tools, do not use only one type]
- insight_forge: Deep insight analysis, automatically decomposes questions and retrieves facts and relationships multi-dimensionally
...
7. [Emphasis] Do not add any headings! Use **bold** to replace sub-section titles"""
...
- Write body text directly, use **bold** to replace sub-section titles
...
class ReportAgent:
    ...
    def _define_tools(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "interview_agents": {
                ...
                "parameters": {
                    "interview_topic": "Interview topic or requirement description (e.g., 'understand students\' views on an event')",
    ...
    def _parse_tool_calls(self, response: str) -> List[Dict[str, Any]]:
        ...
        # Only attempted when format 1 did not match, to avoid false matches in body JSON
    ...
    def _generate_section_react(
        ...
        for iteration in range(max_iterations):
            ...
            if response is None:
                ...
                # If there are iterations remaining, add message and retry
                if iteration < max_iterations - 1:
                    messages.append({"role": "assistant", "content": "(empty response)"})
                    messages.append({"role": "user", "content": "Please continue generating content."})
                    continue
                # Last iteration also returned None, break out of loop for forced conclusion
                break
            ...
            if has_tool_calls and has_final_answer:
                ...
                if conflict_retries <= 2:
                    ...
                    logger.warning(
                        ...
                        "degraded to truncating and executing first tool call"
                    )
...
class ReportManager:
    ...
    def save_section(
        ...
        # Build section Markdown content - clean up potential duplicate titles
    ...
    def _clean_section_content(cls, content: str, section_title: str) -> str:
        ...
        for i, line in enumerate(lines):
            ...
            if heading_match:
                ...
                # Check if this is a heading that duplicates the section title (skip duplicates within first 5 lines)
                if i < 5:
