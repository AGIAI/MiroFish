...
A. **Fallback types (must be included, placed as the last 2 in the list)**:
   - `Person`: Fallback type for any individual person. When a person does not belong to other more specific person types, classify them here.
...
- Various people appear in text, such as "elementary school teachers", "bystanders", "some netizen"
...
class OntologyGenerator:
    ...
    def _build_user_message(
        ...
        if len(combined_text) > self.MAX_TEXT_LENGTH_FOR_LLM:
            ...
            combined_text += f"\n\n...(Original text is {original_length} characters; first {self.MAX_TEXT_LENGTH_FOR_LLM} characters extracted for ontology analysis)..."
