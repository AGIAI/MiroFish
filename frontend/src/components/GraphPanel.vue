    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">Some content is still being processed. You may want to manually refresh the graph later.</span>
...
const renderGraph = () => {
  ...
  // Track which edge index we're at for each node pair
  const edgePairIndex = {}
  ...
  tempEdges.forEach(e => {
    ...
    let curvature = 0
    if (totalCount > 1) {
      // Evenly distribute curvature to ensure clear differentiation
  ...
  const getLinkPath = (d) => {
    ...
    // Perpendicular offset to edge direction, proportional to distance, ensuring curve visibility
    // More edges means larger offset-to-distance ratio
    const pairTotal = d.pairTotal || 1
    const offsetRatio = 0.25 + pairTotal * 0.05 // Base 25%, add 5% per extra edge
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        ...
        if (!d._isDragging && distance > 3) {
          // Only restart simulation on first real drag detected
          d._isDragging = true
