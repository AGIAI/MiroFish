...
const doStartSimulation = async () => {
  ...
  // Reset all state first to avoid being affected by previous simulation
  resetAllState()
...
// Track each platform's previous round for detecting changes and logging
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Determined by checking twitter_completed and reddit_completed status
...
const checkPlatformsCompleted = (data) => {
  ...
  // If at least one platform completed, check if all enabled platforms have completed
